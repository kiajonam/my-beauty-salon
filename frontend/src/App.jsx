...
function PublicSite() {
  ...
  return <div className="site-shell">
    <header className="navbar"><a className="brand" href="#home" onClick={() => setMenuOpen(false)}><span className="brand-mark"><Scissors size={18}/></span><span>My Beauty Salon</span></a><nav className={menuOpen ? 'nav-links open' : 'nav-links'}>{navItems.map(([label,href])=><a key={href} href={href} onClick={()=>setMenuOpen(false)}>{label}</a>)}<button className="nav-booking" onClick={()=>{setBookingOpen(true);setMenuOpen(false);}}>Termin buchen</button></nav><button className="menu-button" aria-label="Menü öffnen" onClick={()=>setMenuOpen(v=>!v)}>{menuOpen?<X/>:<Menu/>}</button></header>
    <main>
      <section id="home" className="hero section-pad"><div className="hero-copy reveal"><div className="eyebrow"><Sparkles size={15}/> Hair. Beauty. You.</div><h1>Dein Look.<br/><em>Dein Moment.</em></h1><p>Modernes Hair & Beauty in Köln. Präzise Schnitte, lebendige Farben und Services, die sich nach dir anfühlen.</p><div className="hero-actions"><button className="primary-button" onClick={()=>setBookingOpen(true)}>Termin buchen <CalendarDays size={18}/></button><a className="text-button" href="#services">Services entdecken <span>↗</span></a></div><div className="hero-meta"><span><Star size={15} fill="currentColor"/> 4,9/5 Kundenbewertung</span><span><Clock3 size={15}/> Mo–Mi 09:00–18:00 · Do–Fr 09:00–19:00</span></div></div><div className="hero-visual reveal delay-1"><div className="hero-orb orb-one"/><div className="hero-orb orb-two"/><div className="portrait-card"><div className="portrait-image" role="img" aria-label="Abstrakte Beauty-Farbwelt"/><div className="floating-card"><span className="floating-icon"><Sparkles size={16}/></span><div><strong>Signature Color</strong><small>individuell abgestimmt</small></div></div></div></div></section>
    ...
    <footer id="contact" className="footer section-pad"><div><a className="brand" href="#home"><span className="brand-mark"><Scissors size={18}/></span><span>My Beauty Salon</span></a><p>Hair & Beauty in Köln.</p></div><div><strong>Kontakt</strong><a href="tel:+492211234567">0221 123 45 67</a><a href="mailto:hello@mybeautysalon.de">hello@mybeautysalon.de</a></div><div><strong>Adresse</strong><span>Köln, Deutschland</span></div><div><strong>Social</strong><a href="#contact">Instagram</a><a href="/impressum">Impressum</a><a href="/datenschutz">Datenschutz</a></div></footer>
