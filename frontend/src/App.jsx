...
export default function App() {
  ...
  useEffect(() => { api.services().then(data => setServices(data.services)).catch(() => {}); }, []);
  ...
  return <div className="site-shell">
    <header className="navbar">
      <a className="brand" href="#home" onClick={() => setMenuOpen(false)}><span className="brand-mark"><Scissors size={18}/></span><span>Barber<span>man</span></span></a>
      <nav className={menuOpen ? 'nav-links open' : 'nav-links'}>
        {navItems.map(([label, href]) => <a key={href} href={href} onClick={() => setMenuOpen(false)}>{label}</a>)}
        <button className="nav-booking" onClick={() => { setBookingOpen(true); setMenuOpen(false); }}>Termin buchen</button>
      </nav>
      <button className="menu-button" aria-label="Menü öffnen" onClick={() => setMenuOpen(v => !v)}>{menuOpen ? <X/> : <Menu/>}</button>
    </header>

    <main>
      <section id="home" className="hero section-pad">
        <div className="hero-copy reveal"><div className="eyebrow"><Sparkles size={15}/> Hair. Beauty. You.</div><h1>Dein Look.<br/><em>Dein Moment.</em></h1><p>Modernes Hair & Beauty in Köln. Präzise Schnitte, lebendige Farben und Services, die sich nach dir anfühlen.</p><div className="hero-actions"><button className="primary-button" onClick={() => setBookingOpen(true)}>Termin buchen <CalendarDays size={18}/></button><a className="text-button" href="#services">Services entdecken <span>↗</span></a></div><div className="hero-meta"><span><Star size={15} fill="currentColor"/> 4,9/5 Kundenbewertung</span><span><Clock3 size={15}/> Mo–Fr 09:00–18:00</span></div></div>
        <div className="hero-visual reveal delay-1"><div className="hero-orb orb-one"/><div className="hero-orb orb-two"/><div className="portrait-card"><div className="portrait-image" role="img" aria-label="Abstrakte Beauty-Farbwelt"/><div className="floating-card"><span className="floating-icon"><Sparkles size={16}/></span><div><strong>Signature Color</strong><small>individuell abgestimmt</small></div></div></div></div>
      </section>
    ...
    <footer id="contact" className="footer section-pad"><div><a className="brand" href="#home"><span className="brand-mark"><Scissors size={18}/></span><span>Barber<span>man</span></span></a><p>Hair & Beauty in Köln.</p></div><div><strong>Kontakt</strong><a href="tel:+492211234567">0221 123 45 67</a><a href="mailto:hello@barberman.de">hello@barberman.de</a></div><div><strong>Adresse</strong><span>Köln, Deutschland</span></div><div><strong>Social</strong><a href="#contact"><Instagram size={17}/> Instagram</a></div></footer>
...
function BookingModal({ services, onClose }) {
  ...
  async function submit() {
    ...
    try { await api.book({ serviceId:Number(serviceId), date, time, ...form }); setStep(4); } catch (error) { setMessage(error.message); } finally { setLoading(false); }
  }

  return <div className="modal-backdrop" onMouseDown={onClose}><div className="booking-modal" onMouseDown={e => e.stopPropagation()}><button className="modal-close" aria-label="Schließen" onClick={onClose}><X/></button>
    {step < 4 ? <><div className="eyebrow">Online-Termin</div><h2>{step===1?'Was möchtest du buchen?':step===2?'Wann passt es dir?':'Noch kurz deine Daten.'}</h2>
      {step===1 && <div className="booking-options">{services.map(item => <button key={item.id} className={Number(serviceId)===item.id?'booking-option active':'booking-option'} onClick={()=>setServiceId(item.id)}><span>{item.name}</span><strong>{money(item.priceFrom)}</strong></button>)}</div>}
      {step===2 && <div className="booking-fields"><label>Datum<input type="date" min={new Date().toISOString().slice(0,10)} value={date} onChange={e=>setDate(e.target.value)}/></label><div><span className="field-label">Uhrzeit</span>{slots.length ? <div className="time-grid">{slots.map(t=><button key={t} className={time===t?'time-slot active':'time-slot'} onClick={()=>setTime(t)}>{t}</button>)}</div> : <p className="empty-state">Für diesen Tag sind aktuell keine Zeiten verfügbar.</p>}</div></div>}
      {step===3 && <div className="booking-fields"><label>Name<input name="name" value={form.name} onChange={update} placeholder="Vor- und Nachname"/></label><label>E-Mail<input name="email" type="email" value={form.email} onChange={update} placeholder="name@beispiel.de"/></label><label>Telefon<input name="phone" value={form.phone} onChange={update} placeholder="0176 12345678"/></label><label>Nachricht<input name="note" value={form.note} onChange={update} placeholder="Optional"/></label></div>}
      {message && <p className="form-error">{message}</p>}
      <div className="modal-footer"><span>Schritt {step} von 3</span><button className="primary-button" disabled={!canContinue || loading} onClick={()=> step<3 ? setStep(step+1) : submit()}>{loading?'Wird gesendet…':step<3?'Weiter':'Termin anfragen'} {step<3 && '→'}</button></div></>
      : <div className="booking-success"><Sparkles size={40}/><div className="eyebrow">Vielen Dank</div><h2>Termin angefragt.</h2><p>Deine Anfrage wurde gespeichert. Wir melden uns zur Bestätigung bei dir.</p><button className="primary-button" onClick={onClose}>Schließen</button></div>}
  </div></div>;
}
